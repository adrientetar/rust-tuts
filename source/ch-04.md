...
It might seem a bit early to talk about those topics but it is a good spot for Rust because these features are often used while writing the initial code. We will also talk about error handling, which you will encounter sooner or later as it is quite different from most of today's programming languages.
...
Besides `fail!` that halts the program and displays the error message passed as argument, tests are often made of assertions: the `assert!` macro takes a boolean expression as parameter, does nothing if it evaluates to true, fails if it proves to be false. `assert_eq!` is a variant takes 2 variables as parameters and asserts equality. There is also `assert_approx_eq!` for floats.
...
Well, Rust has this, but more powerful in the name of `match`, pattern matching.
